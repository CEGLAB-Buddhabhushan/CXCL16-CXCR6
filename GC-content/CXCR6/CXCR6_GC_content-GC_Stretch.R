...
library(dplyr)
library(ggplot2)
df<- read.table("CXCR6_GC_content-GC_Stretch.tsv", header =T, sep = '\t')
...
p1 <- ggplot() +
  geom_point(data = merge_df[350:41243, ], aes(x = GC_Stretch, y = GC_content),alpha = 0.02, color = "gray")+
  geom_point(data = merge_df[1:349, ], aes(x = GC_Stretch, y = GC_content, color = Group, shape = Group), size = 3) +
  scale_color_manual(values = color_mapping, name = "Group") +
  scale_shape_manual(values = shape_mapping, name = "Group") +
  labs(title = "CXCR6 GC content vs G/C-stretch length",
  ...
  theme_classic()
png("CXCR6_GC_content-GC_Stretch.png", units="in", width=16, height=9, res=600)
print(p1)
dev.off()
